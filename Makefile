...
pkgdatadir = $(datadir)/yeahreumcore
pkgincludedir = $(includedir)/yeahreumcore
pkglibdir = $(libdir)/yeahreumcore
pkglibexecdir = $(libexecdir)/yeahreumcore
...
CONFIG_HEADER = $(top_builddir)/src/config/yeahreum-config.h
CONFIG_CLEAN_FILES = libyeahreumconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libyeahreumconsensus.pc.in \
	...
	$(top_srcdir)/src/config/yeahreum-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/Yeahreum/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/Yeahreum/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/Yeahreum/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/Yeahreum/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = yeahreum-cli
BITCOIN_DAEMON_NAME = yeahreumd
BITCOIN_GUI_NAME = yeahreum-qt
BITCOIN_TX_NAME = yeahreum-tx
...
COPYRIGHT_HOLDERS_FINAL = The Yeahreum Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = Yeahreum Core
COPYRIGHT_YEAR = 2019
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/Yeahreum/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = yeahreumcore
PACKAGE_BUGREPORT = https://github.com/yeahreumpay/yeahreum/issues
PACKAGE_NAME = Yeahreum Core
PACKAGE_STRING = Yeahreum Core 1.0.0
PACKAGE_TARNAME = yeahreumcore
PACKAGE_URL = https://yeahreum.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/Yeahreum
abs_srcdir = /tmp/Yeahreum
abs_top_builddir = /tmp/Yeahreum
abs_top_srcdir = /tmp/Yeahreum
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/Yeahreum/build-aux/install-sh
...
pkgconfig_DATA = libyeahreumconsensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/yeahreum-cli.bash-completion \
	       $(top_srcdir)/contrib/yeahreum-tx.bash-completion \
	       $(top_srcdir)/contrib/yeahreumd.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_yeahreum_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_yeahreum_coverage.info test_yeahreum.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Yeahreum-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/yeahreum-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/yeahreum-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/yeahreum-config.h
$(top_srcdir)/src/config/yeahreum-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/yeahreum-config.h src/config/stamp-h1
libyeahreumconsensus.pc: $(top_builddir)/config.status $(srcdir)/libyeahreumconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/Yeahreum-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Yeahreum-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Yeahreum-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_yeahreum.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_yeahreum -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_yeahreum_filtered.info: test_yeahreum.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_yeahreum_filtered.info
...
#test_yeahreum_coverage.info: baseline_filtered_combined.info test_yeahreum_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_yeahreum_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_yeahreum_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_yeahreum_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_yeahreum.coverage/.dirstamp:  test_yeahreum_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_yeahreum.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_yeahreum.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
